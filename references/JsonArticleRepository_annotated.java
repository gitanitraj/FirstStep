/* =============================================================================
 * ANNOTATED REFERENCE — backend/.../originals/repository/JsonArticleRepository.java
 * Slice K. See references/decisions.md Decision 048.
 * =============================================================================
 *
 * WHAT IT DOES: loads app/data/articles.json once at application-ready, external
 * file first with a classpath fallback — JsonFlyerRepository's shape, followed
 * rather than improved on so the seventh loader reads like the other six.
 *
 * IT LOADS EVERY ARTICLE, INCLUDING DRAFTS
 * ----------------------------------------
 * See ArticleRepository's mirror: filtering here would look like defence in depth
 * and would in fact collapse "withheld" into "absent", removing the population a
 * future editorial queue needs.
 *
 * WHY A MISSING FILE IS NOT AN ERROR
 * ----------------------------------
 * TaxonomyService treats a missing taxonomy.json as fatal, and that is correct
 * there — a deployment with no categories is broken. This is the opposite case.
 * First Step Originals PREDATES articles: the homepage panel carried FAQs before
 * Slice K and works unchanged without a single article. A missing articles.json
 * is a valid state, including during a rollback, so it logs and carries on.
 *
 * The general rule this pair illustrates: FAIL HARD WHEN ABSENCE MEANS BROKEN,
 * FAIL SOFT WHEN ABSENCE MEANS "THAT FEATURE ISN'T IN USE YET."
 *
 * WHY THE STARTUP LOG REPORTS A BREAKDOWN RATHER THAN A COUNT
 * -----------------------------------------------------------
 *     Loaded articles from /data/articles.json (3 records, 1 approved for
 *     public serving)
 *
 * "3 records" hides the only number that matters operationally. The gap between
 * the two figures is the review queue, visible at every boot without asking
 * anyone to run a query — and it made the boundary's first live verification
 * immediate rather than inferred.
 * ============================================================================= */
